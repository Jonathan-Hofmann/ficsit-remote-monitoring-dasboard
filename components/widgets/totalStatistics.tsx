...
export const TotalStatistics = ({ SFRM }: { SFRM: SubManager}) => {
    ...
    return(
        <>
            <Card className="">
                {circuits && <PieCompontent1 data={circuits}/>}
            </Card>
            <Card className="">
                {circuits && <EnergyPieConsumption data={circuits}/>}
            </Card>
            {circuits && <AreaChartCompare2ValsBig className="row-span-2" data={circuits} dataKey1="PowerConsumed" dataKey2="PowerCapacity"/>}
            <Card>
                <CardContent>
                    <p className="text-3xl font-bold">{data?.length}</p>
                    <p className="text-sm text-muted-foreground">Connected Power Circuits</p>
                </CardContent>
            </Card>
            <Card>
                <CardContent>
