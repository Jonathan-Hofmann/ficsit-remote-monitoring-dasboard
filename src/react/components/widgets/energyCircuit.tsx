    ...
    <AccordionItem value={index.toString()}>
          ...
          {generators[circuit.CircuitID] ? (
            <>
              <div className="bg-background group-hover:bg-zinc-100 group-hover:dark:bg-background dark:bg-zinc-900 px-2 py-1 rounded-md border flex flex-row items-center">
                <p>
                  {Math.round(circuit.PowerConsumed).toLocaleString()}{" "}
                ...
                />
                <p>
                  {Math.round(circuit.PowerCapacity).toLocaleString()}{" "}
                  <span className="text-sm text-muted-foreground">MW</span>
                </p>
              </div>
              <div className="bg-background group-hover:bg-zinc-100 group-hover:dark:bg-background dark:bg-zinc-900 px-2 py-1 rounded-md border flex flex-row justify-start">
                <p>
                  ...
                  {Math.round(circuit.PowerMaxConsumed).toLocaleString()}{" "}
                  <span className="text-sm text-muted-foreground">MW</span>
                </p>
              </div>
              <div
                className={cn([
                  " px-2 py-1 rounded-md border flex flex-row items-center gap-2 bg-background dark:bg-zinc-900 group-hover:bg-zinc-100 group-hover:dark:bg-background",
                ])}
              >
      ...
      <AccordionContent>
        <div className="grid grid-cols-4 gap-4 py-4 px-2">
          <EnergyConsumptionWidgetWithData
            data={circuit}
