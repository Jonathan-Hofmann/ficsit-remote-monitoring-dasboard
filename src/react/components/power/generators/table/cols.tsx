...
import { FuelInventoryItem } from "@/types/apis/dataTransferObject/generatorsDto"
...
export const powerGeneratorsCols: ColumnDef<GeneratorFm>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if(tmp >= 0){
        return(
          <Badge variant={"secondary"}>Circuit {tmp}</Badge>
        )
      } else {
        return(
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const prod = parseFloat(row.getValue("loadPercentage"))
  ...
  },
  {
    ...
    cell: ({ row }) => (<NumberCell row={row} suffix="%" selector="currentPotential" />)
  },
  {
